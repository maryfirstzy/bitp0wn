...
Q3x = (
    (x**9 - 672 * x**6 + 2352 * x**3 + 21952) * inv(9 * x**2 * (x**3 + 28) ** 2, P)
) % P
...
# Double Qx and add G
Q2xp1 = (
    ((Q2[0] * G[0]) * (Q2[0] + G[0]) - 2 * Q2[1] * G[1] + 14)
    * inv((Q2[0] - G[0]) ** 2, P)
) % P
...
assert (
    x**5 % P
    == ((x**3 + 7) * (x**3 + 7) * (8 * x + 4 * Q2x) * (x - 4 * Q2x) * inv(567, P)) % P
)
# Eq 3.
...
assert ((G[1] - Q2y) ** 3 * inv(G[0] - Q2x, P) ** 3) % P == (
    ...
    + 24 * G[1] * y**11
    - (192 * G[1] ** 2 + 378) * y**10
    + (512 * G[1] ** 3 + 6048 * G[1]) * y**9
    - (24192 * G[1] ** 2 + 43659) * y**8
    + 317520 * G[1] * y**7
    + (254016 * G[1] ** 2 - 1000188) * y**6
    - 8001504 * G[1] * y**5
    + 57760857 * y**4
    + 42007896 * G[1] * y**3
    - 661624362 * y**2
    ...
    (
        -8 * y**11
        + (96 * G[0] * x**2 - 384 * x * G[0] ** 2 + 512 * G[0] ** 3 + 1568) * y**9
        + (-12096 * G[0] * x**2 + 24192 * x * G[0] ** 2 - 105840) * y**7
        + (381024 * G[0] * x**2 + 2667168) * y**5
        - 14002632 * y**3
    ),
    P,
) % P
...
Q2yp1 = (
    3 * (G[1] - Q2y) * Q2x * inv(G[0] - Q2x, P)
    - (G[1] - Q2y) ** 3 * inv(G[0] - Q2x, P) ** 3
    - 2 * Q2y
    + G[1]
) % P
...
assert (_x - Z1) * (_x**3 + G * _x**2 + H * _x + I) % P == (
    _x**4 - 4 * Q2[0] * _x**3 - 56 * _x - 28 * Q2[0]
) % P
assert (_x - Z1) * (
    Z1**3
    + Z1**2 * (_x - 4 * Q2[0])
    + Z1 * (_x**2 - 4 * Q2[0] * _x)
    + (_x**3 - 4 * Q2[0] * _x**2 - 56)
) % P == (_x**4 - 4 * Q2[0] * _x**3 - 56 * _x - 28 * Q2[0]) % P
...
assert (
    ...
    * (
        x**3
        - (res1 + res2 + res3) * x**2
        + (res1 * res2 + res1 * res3 + res2 * res3) * x
        - res1 * res2 * res3
    )
    % P
    == (x**3 + G * x**2 + H * x + I) % P
...
_q = (
    (2 * (_b**3) - 9 * (_a) * (_b) * (_c) + 27 * (_a**2) * (_d)) * inv(27 * (_a**3), P)
) % P
...
assert (
    _x**4
    + (126 - 24 * Q2[1] ** 2) * _x**2
    + (-64 * Q2[1] ** 3 + 504 * Q2[1]) * _x
    + (-48 * Q2[1] ** 4 + 504 * Q2[1] ** 2 - 1323)
) % P == 0
